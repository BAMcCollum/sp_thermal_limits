...
maxtemp_mindepth_tipping_point_plotminmax <- thermal_indicies2 %>% 
  ...
  geom_point(aes(y=BO21_tempmax_bdmin_min.x),  color = color2, alpha = 1, size = 3)+
  geom_point(aes(y=BO21_tempmax_bdmin_max.x), color = color2, alpha = 1, size = 3) +
  geom_segment(aes(xend=gen_spp,
                   y=BO21_tempmax_bdmin_min.x,
                   yend=BO21_tempmax_bdmin_max.x), color = color2, alpha=1, linewidth = 1.5) +
